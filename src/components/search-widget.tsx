  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent 
        ...
        sideOffset={8}
      >
        <div className="relative px-4 sm:px-6 pt-4 sm:pt-6 pb-3 sm:pb-4 border-b border-neutral-200 bg-gradient-to-r from-white to-neutral-50">
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-3 sm:py-4 bg-gradient-to-b from-white to-neutral-50">
          <div className="space-y-3 sm:space-y-4 min-h-[180px]">
                ...
                <div className="space-y-2 mt-4 sm:mt-6">
                  ...
                  {suggestedQuestions.map((question, index) => (
                    <button
                      ...
                      disabled={isLoading}
                      className="w-full text-left px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg bg-neutral-100 border-2 border-neutral-200 hover:border-blue-500/50 hover:shadow-md transition-all duration-200 text-xs sm:text-sm text-neutral-700 hover:text-blue-600 disabled:opacity-50 disabled:cursor-not-allowed group"
                    >
                      <span className="flex items-center gap-2">
                        <Sparkles className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-blue-600 group-hover:text-blue-700 flex-shrink-0" />
                ...
                <div
                  className={`rounded-2xl px-4 py-3 max-w-[90%] shadow-sm transition-all duration-200 hover:shadow-md ${
                    message.role === 'user'
                      ...
                      : 'bg-neutral-100 text-neutral-900 border-2 border-neutral-200 rounded-bl-sm'
                  }`}
                >
                  {message.role === 'user' ? (
                    ...
                    <div className="text-sm break-words markdown-content prose prose-sm max-w-none">
                      <ReactMarkdown
                        components={{
                          ...
                          pre: ({ children }) => (
                            <pre className="bg-neutral-200 text-neutral-900 p-3 rounded-lg text-xs overflow-x-auto my-3 font-mono border border-neutral-300">
                              {children}
                            </pre>
                          ),
                          blockquote: ({ children }) => (
                          ...
                          a: ({ href, children }) => (
                            <a 
                              href={href} 
                              className="text-blue-600 hover:text-blue-700 underline decoration-blue-500 hover:decoration-blue-600 transition-colors" 
                              target="_blank" 
            ...
            {isLoading && (
              <div className="flex items-start gap-2 animate-slide-in">
                <div className="bg-neutral-100 text-neutral-900 border-2 border-neutral-200 rounded-2xl rounded-bl-sm px-4 py-3 shadow-sm">
        ...
        <div className="px-4 sm:px-6 pb-4 sm:pb-6 pt-3 sm:pt-4 border-t border-neutral-200 bg-white">
          <form onSubmit={handleSubmit} className="flex gap-2">
            <input
              value={input}
              onChange={(event) => setInput(event.target.value)}
              placeholder="Digite sua pergunta..."
              className="flex-1 rounded-xl border-2 border-neutral-200 bg-neutral-50 px-3 sm:px-4 py-2.5 sm:py-3 text-sm text-neutral-900 placeholder-neutral-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
              disabled={isLoading}
              autoComplete="off"
            />
            <button 
